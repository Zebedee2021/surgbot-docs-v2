...
WARMUP_STEPS = 500               # 初始化热身步数（让臂收敛到 home 姿态）
...
<mujoco model="surgbot_ur5_approx">
  ...
  <worldbody>
    ...
    <!--
      正面全景：从右前方斜上方俯视，可同时看到
      机械臂全链（底座→大臂→前臂→腕→夹爪）和托盘。
      pos=(0.6,-1.3,0.6)  lookat≈(0.28,-0.08,0.30)
      xyaxes 由右手法则推导：
        camera_X = look × world_up = (0.967, 0.254, 0)
        camera_Y = (-camera_Z) × camera_X = (-0.059, 0.223, 0.973)
    -->
    <camera name="front_view"
            pos="0.600 -1.300 0.600"
            xyaxes="0.967 0.254 0  -0.059 0.223 0.973"/>
